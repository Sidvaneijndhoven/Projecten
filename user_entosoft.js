    ...
    else
    {
        When("Log in as user1 start the batch and then login as user2 and edit batch",(data) => {    
            ...
            data.hashes().forEach((element) => {
                const weighIndividual = element["Weigh indivual"];
        
                if(weighIndividual.toLowerCase() == "yes")
                {
                    cy.get('label[for="id_weigh_individual_ovisites"]').click()
                }
                else
                {
                    cy.log("Weigh individual: No")
                } 
